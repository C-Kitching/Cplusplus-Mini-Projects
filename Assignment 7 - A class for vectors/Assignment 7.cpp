...
class Minkowski : private vector 
{
...
public:
	...
	particle() : mass{ 0 }, vector_3{ vector(3) }, vector_4{ vector(4) } {};  // default constructor
	particle(double, vector&, vector&);  // parameterized constructor
	~particle() {};  // destructor

	// access functions
	double& component(int) const;
...
particle::particle(double m, vector& input_vector_3, vector& input_vector_4) : mass{ 0 }, vector_3{ vector(3) }, vector_4{ vector(4) }
{
...
{
	const double c{ 3e8 };  // speed of light
	double momentum{ gamma * mass * vector_3.magnitude() * c };
	return momentum;
}

// particle member function to return 4 vector components
double& particle::component(int component) const 
{
	return vector_4[component];
}
...
std::ostream& operator<<(std::ostream& os, const particle& particle_input) 
{
	...
	vector vector_4{ particle_input.vector_4 };
...
int main() {
	...
	vector velocity(100000, 100000, 100000);
	Minkowski unboosted(15,1,15,2);
	...
	std::cout << "ct = " << proton.component(1) << "\nx = " << proton.component(2)
		<< "\ny = " << proton.component(3) << "\nz = " << proton.component(4) << std::endl;
